...
> 🧭 **Obsidian で開くなら [入口ノート `index.md`](index.md) から。** 会社全体をリンクでたどれます。
        ...
        hayato（人間・最終判断）
           └─ AI部長（hayato代理・一次判断）
                └─ 🕊️ つばさ（AI秘書・窓口/実務）
                     │  窓口 / 要件整理 / 振り分け / 集約 / 報告
      ┌───────────┬───────────┼───────────┬───────────┐
   📅 スケジュール  ✉️ メール    📁 資料      💻 開発     🏢 各子会社
      担当          担当         担当         担当（レボ）  （companies/）
```

- **hayato** が最終判断／**AI部長**が hayato 代理で一次判断／**つばさ**は概念不変で実務を回す
- 決裁の線引き＝ [ガバナンス](docs/governance.md)
...
| ファンディング / 診断 実装 | codex（Codex） | レボファンディング（レボアート含む）・レボリスト診断 |
...
### 各プロジェクトは「子会社（サブカンパニー）」化

担当を1人置くのではなく、**各プロジェクトを社長＋部署を持つ独立会社**として運営する。
本社（つばさ）が発注 → 各社の社長が内部に割り振り → 統合して本社へ報告。

→ 一覧と各社の定義は **[子会社一覧](companies/README.md)**（6社）を参照。
